...
data["5y_ROIC_stability"] = roic.groupby(data["Ticker"]).transform(lambda x: x.rolling(5).std())
...
data["net_debt_change_3y"] = (net_debt - net_debt.groupby(data["Ticker"]).shift(3)) / average_assets
...
data["Score2"] = data.groupby(["Year", "Sector"])["5y_ROIC_stability"].rank(pct=True, ascending=False)
...
data["Score7"] = data.groupby(["Year", "Sector"])["net_debt_change_3y"].rank(pct=True, ascending=False)
data["Score8"] = data.groupby(["Year", "Sector"])["ebit_yield"].rank(pct=True)
data["Score9"] = data.groupby(["Year", "Sector"])["fcf_yield"].rank(pct=True)
data["Score10"] = data.groupby(["Year", "Sector"])["volatility"].rank(pct=True, ascending=False)
...
winners.to_csv("data/processed/winners_sector_nobias.csv", index=False)
